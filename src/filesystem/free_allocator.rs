use crate::{
    BlockDevice, Error,
    filesystem::{
        Addr, Deserializable, MAX_DATA_BLOCKS, Range, Serializable, StaticReadFromDevice,
        WriteToDevice,
    },
};

use super::{Block, Node, free::Free};
...
impl FreeBlockAllocator {
    pub const RANGE: Range = Node::RANGE.next(MAX_DATA_BLOCKS as Addr / Free::SLOTS as Addr);
    pub const LEN: usize = Self::RANGE.len();
    ...
    D: BlockDevice,
{
    fn write_to_device(&self, out: &mut D) -> Result<(), Error> {
        ...
        for (pos, free) in self.inner.iter().enumerate().filter(|(pos, _)| self.dirty[*pos]) {
            ...
            out.write_block(Self::RANGE.nth(pos as u32), &block)?;
        }
    ...
    D: BlockDevice,
{
    ...
    fn read_from_device(device: &mut D) -> Result<Self, Error> {
        ...
        for i in 0..Self::LEN {
            let mut block = Block::new();
            device.read_block(Self::RANGE.nth(i as u32), &mut block)?;
            allocator.inner[i] = Free::deserialize(&block)?;
        }
        Ok(allocator)
    }
}
