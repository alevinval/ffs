...
impl FileName {
    ...
    pub const fn buffer() -> [u8; MAX_FILENAME_LEN] {
        [0; MAX_FILENAME_LEN]
    }
...
impl Deserializable<FileName> for FileName {
    fn deserialize(buf: &[u8]) -> Result<Self, Error> {
        if buf.len() < MAX_FILENAME_LEN + 1 {
            return Err(Error::BufferTooSmall { expected: MAX_FILENAME_LEN + 1, found: buf.len() });
        }
        ...
        let mut bytes = [0u8; MAX_FILENAME_LEN];
        bytes[..len].copy_from_slice(&buf[1..len + 1]);
        Ok(Self { bytes, len })
    }
}
