use crate::Error;
use crate::filesystem::{Addr, Block, Deserializable, Serializable};
use crate::io::Write;
...
impl Serializable for Free {
    ...
    fn serialize<W: Write>(&self, out: &mut W) -> Result<(), Error> {
...
impl Deserializable<Free> for Free {
    ...
    fn deserialize(buf: &[u8]) -> Result<Self, Error> {
        if buf.len() < Block::LEN {
            return Err(Error::BufferTooSmall { expected: Block::LEN, found: buf.len() });
        }
        let mut free = Self::new();
        free.inner.copy_from_slice(&buf[0..Block::LEN]);
        Ok(free)
    }
}
...
mod test {
    ...
    fn serde_symmetry() {
        ...
        let actual = Free::deserialize(&block).expect("should deserialize");
