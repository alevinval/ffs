...
pub struct DirEntry {
    pub name: FileName,
    pub dirs: [Addr; MAX_CHILD_DIRS],
    pub files: [FileEntry; MAX_CHILD_FILES],
}
...
impl DirEntry {
    pub fn root() -> Self {
        Self {
            ...
            dirs: [const { 0 }; MAX_CHILD_DIRS],
            files: [const { FileEntry::empty() }; MAX_CHILD_FILES],
        }
    }

    pub const fn new(name: FileName) -> Self {
        let dirs = [const { 0 }; MAX_CHILD_DIRS];
        let files = [const { FileEntry::empty() }; MAX_CHILD_FILES];
        Self { name, dirs, files }
    }

    pub fn load<D: BlockDevice>(device: &mut D, idx: Addr) -> Result<DirEntry, Error> {
        let range = Layout::TABLE;
...
impl Deserializable<DirEntry> for DirEntry {
    fn deserialize<R: Read>(reader: &mut R) -> Result<Self, Error> {
        ...
        let mut dirs = [0; MAX_CHILD_DIRS];
        for dir in dirs.iter_mut() {
            *dir = reader.read_u32()?;
        }

        let mut files = [const { FileEntry::empty() }; MAX_CHILD_FILES];
