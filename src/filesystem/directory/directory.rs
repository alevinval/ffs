...
impl Directory {
    ...
    pub fn add_file<D>(&mut self, device: &mut D, file_name: FileName) -> Result<Entry, Error>
    where
        D: BlockDevice,
    {
    ...
    pub fn remove_file<D>(&mut self, device: &mut D, file_name: &FileName) -> Result<(), Error>
    where
        D: BlockDevice,
    {
        for idx in 0..Self::LEN as Addr {
    ...
    D: BlockDevice,
{
    ...
    let sector = Layout::TABLE.nth(idx);
    device.write_block(sector, &mut block)?;
    Ok(())
}
    ...
    D: BlockDevice,
{
    ...
    fn next(&mut self) -> Option<Self::Item> {
        for _ in self.pos..Directory::LEN {
