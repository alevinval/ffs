...
use crate::{
    BlockDevice, Error,
    filesystem::{Addr, TreeNode, directory::tree::find_and_then},
};
...
#[cfg(feature = "std")]
pub fn print_tree_stdout<D>(device: &mut D, base_path: &str, depth: usize) -> Result<(), Error>
where
    D: BlockDevice,
{
    ...
    print_tree(device, base_path, depth, &mut txt)?;
    println!("{txt}");
    Ok(())
}
...
mod tests {
    ...
    use crate::{
        disk::MemoryDisk,
        filesystem::{SerdeLen, directory::Tree, layout::Layout},
    };
    ...
    fn test_print_tree() {
        ...
        assert_eq!(Ok(()), print_tree(&mut device, "", 0, &mut actual));
        assert_eq!("$/\n", &actual);
...
";
    ...
    fn test_print_tree_relative() {
        ...
        let mut actual = String::new();
        assert_eq!(Ok(()), print_tree(&mut device, "dir1/dir2", 0, &mut actual));
        let expected = "../
  dir3/
    file.txt
";
    ...
    fn test_print_tree_relative_and_max_depth() {
        ...
        let mut actual = String::new();
        assert_eq!(Ok(()), print_tree(&mut device, "dir1", 2, &mut actual));
        let expected = "../
  dir2/
    dir3/
  dir3/
...
";
    ...
    fn test_print_file_fails() {
        ...
        let result = print_tree(&mut device, "dir1/file.txt", 0, &mut out);
